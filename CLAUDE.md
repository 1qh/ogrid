...
- The react display-name autofix CORRUPTS a `createContext` whose result lacks a `.displayName`, then CRASHES the whole gate. `fix` rewrites `const Ctx = createContext(...)` into a broken `const\nCtx.displayName = 'Ctx'; useSt = ...` — it DROPS the `const` initializer and merges the next declaration — after which the comment-strip pass throws `comment strip cannot parse the source: Missing initializer in const declaration` and no linter runs at all. Give every `createContext` its OWN displayName on the next line BEFORE running `fix` (`const Ctx = createContext<T>(null)` then `Ctx.displayName = '...'`), so the autofix has nothing to add. If the gate is already crashing this way, isolate the broken line with `oxc-parser` `parseSync` (the crash names no file/line), repair by hand to a valid `const … =` + separate displayName assignment, and prefer `check` (read-only) over `fix` on a file holding a bare `createContext`. Same pass also silently strips a file-top `/* oxlint-disable … */` and a non-directive doc comment — re-add and re-verify after any `fix` on such a file.
...
- Set a session cookie’s `Secure` flag from the request SCHEME (`x-forwarded-proto` else the URL protocol), never from `NODE_ENV`. Why: a browser silently drops a `Secure` cookie sent over plain HTTP, so `secure: NODE_ENV === 'production'` breaks the session on any prod deploy served over HTTP on a real host — and it passes every local test because `http://localhost` is a browser secure-context exemption, so only a live drive on a non-localhost HTTP host catches it.

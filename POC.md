...
**Narrow→wide transition re-measurement:** When switching from compact to freeform mode for the first time (viewport crosses above 1200px), the grid triggers a re-measurement of non-`resizedIds` items. At the wider viewport, content reflows (text unwraps), producing shorter heights. ResizeObserver fires, `computedMinH` updates, and `h` is recalculated for non-user-resized items. After re-measurement completes, `computeLayout` re-runs to verify no overlaps exist with the new heights and reposition items if needed (user-positioned items that overlap due to height changes get pushed down, same as initial auto-sizing). The re-measurement window uses the same 200ms idle / 2s cap.
...
**minW/minH sync mechanism:** When ResizeObserver fires (content changed), the observer callback updates both the `measurementsRef` (for constrainSize) AND queues a `computedMinH`/`computedMinW` update. All queued updates are flushed in a single `requestAnimationFrame` callback as ONE `setState` call — updating all changed items in the layout array at once. This produces at most one React re-render per RAF frame, regardless of how many items changed. Worst case during measurement window: if all 25 items' observer callbacks fire in the same RAF, each round is 1 setState → 1 RGL re-render. With a 3-update cap per item, worst case is 3 rounds total. If items stabilize at different rates across different RAFs, the round count could be higher (each RAF with changed items triggers a re-render). POC must log actual re-render count during measurement at 25 items. Pass/fail: total measurement-phase re-renders must be ≤10 for 25 items. After the measurement window closes, `computedMinH`/`computedMinW` updates are the ONLY state changes from observer callbacks — no `w`/`h` changes, so RGL repositioning is not triggered.
...
**Resize-overlap prevention:** Test: resize item A to be larger such that it would overlap item B. The expected ordering: RGL calls `constrainSize` first (which enforces content minimum), then applies its own collision detection to the constrained size. `constrainSize` only handles content minimums — it does NOT need to be aware of neighbors. RGL handles overlap prevention after. Verify this ordering in the POC by logging both `constrainSize` calls and `onResize` callbacks during a resize-into-neighbor scenario. **Fallback if ordering is reversed** (collision detection before constrainSize): `constrainSize` would need to also enforce maximum sizes to prevent overlap. In this case, use the `layout` parameter (available in constrainSize context) to compute maximum `w`/`h` based on neighbor positions. Content minimum takes precedence if min > max (item can't shrink below content, resize is capped).
...
### Pre-POC investigation (partially completed)

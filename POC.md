...
**How to verify:** Render a 4-column grid with an item at `x: 3`. Change to 3 columns. RGL's `correctBounds` clamps `x` (moves items left to fit) but does NOT resolve overlaps between non-static items — it only adjusts `x` position. In freeform mode (`compactType: null`), this means two items clamped to the same position WILL overlap. **Our solution:** after column count change, run `computeLayout` on all items whose `x` was clamped (their positions are now potentially invalid). `computeLayout` resolves overlaps by pushing items down, same as initial auto-placement. A dev warning is emitted for repositioned items. Test with multiple items at the boundary to confirm no overlaps after reflow.

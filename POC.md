...
**Flag persistence:** `userPositioned` and `userResized` are stored in a separate `Map<string, {positioned: boolean, resized: boolean}>` managed by the grid component, NOT inside RGL's layout array (RGL may not preserve custom fields through `onLayoutChange`). The grid's `onLayoutChange` callback emits both the layout array AND the flags map, so consumers can persist both. On re-mount, consumers pass both back as props.

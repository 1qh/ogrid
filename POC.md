...
POC test: a table with no config cannot shrink below its content height. A chart with `minH: 1` can shrink to 1 row. A chart with no config is treated as fixed content (protected). This is correct — the developer must opt in to free shrinking because the grid can't know which components handle arbitrary sizing gracefully.
...
**Flag persistence:** `userPositioned` and `userResized` are derived from the layout config, not stored separately. A positioned item has both `x` and `y` defined in the config. A resized item has an explicit `h` in the config (distinguished from auto-computed `h` because the grid only writes `h` to the emitted layout AFTER the user resizes — auto-computed heights are not persisted until user interaction). This means: the layout array IS the single source of truth. No separate flags map needed. An item with `{x: 2, y: 0, w: 3, h: 4}` in the config is both positioned and sized. An item with `{w: 3}` is unpositioned and auto-sized. The grid emits the full layout (with positions and sizes) via `onLayoutChange` only after user interaction — consumers persist this single object.

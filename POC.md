...
**Near-zero measurement detection:** If an item's measured natural height is below 1 grid row (e.g., a chart with `flex-1 min-h-0` that collapses under `height: auto`), the grid emits a development-mode warning: "item 'X' measured at Npx natural height (less than 1 row). Set explicit `minH` if this is a flexible component, or check your CSS." The item gets `computedMinH: 1` (minimum possible). This warns developers that "fixed content protection" is meaningless for components that collapse — they should set `minH: 1` explicitly to make the intent clear.
...
**Mode switch behavior:** Freeform layout is canonical, stored in a ref (`freeformLayoutRef`) that is ONLY updated when `compactType === null`. Compact layout is DERIVED on every render — not by calling RGL's internal compact function directly, but by passing freeformLayoutRef as the `layout` prop with `compactType={'vertical'}`. RGL handles compaction internally when it receives a layout and a compactType. No need to call any internal utility — just change the `compactType` prop and RGL compacts the layout. No separate state variable. `onLayoutChange` is guarded: it reads `compactType` from a ref (not state) so the guard always sees the latest value regardless of closure staleness. If `compactTypeRef.current !== null`, the callback does NOT update `freeformLayoutRef`. This prevents RGL's internal compaction from overwriting freeform positions, even during rapid viewport resize transitions. Switching back above 1200px restores freeformLayoutRef as-is. Drag and resize are DISABLED in compact mode (`isDraggable={false}` `isResizable={false}`) — this is a read-only responsive view. POC test: switch to compact mode, switch back, verify freeform positions are preserved exactly.

If users need to edit layout on mobile, that's a future feature (separate mobile layout config), not a POC concern.

**Initial load below 1200px:** If the viewport is below 1200px on first load, the measurement window still runs (measuring content at the current viewport width). Heights are auto-sized and stored in `freeformLayoutRef`. The grid renders with `compactType='vertical'` from the start. When the viewport widens above 1200px, `freeformLayoutRef` is used as-is (with measured heights). This means auto-sizing works the same regardless of initial viewport width — the measurement always targets `freeformLayoutRef`.

**Compact mode column count:** Stays at the developer-configured column count. Items with `w` larger than available space are capped to `cols`. The vertical compaction algorithm handles repositioning — items keep their `w` but get new `x`/`y` to eliminate gaps and resolve overlaps. The column count does NOT change at the breakpoint — only the compaction mode changes. Minimum supported viewport width for compact mode: 768px (tablet). Below 768px, column count is too high for usable layouts — a separate mobile layout strategy (e.g., single column) is a future feature, not a POC concern.
...
**minW/minH sync mechanism:** When ResizeObserver fires (content changed), the observer callback updates both the `measurementsRef` (for constrainSize) AND queues a `computedMinH`/`computedMinW` update. All queued updates are flushed in a single `requestAnimationFrame` callback as ONE `setState` call — updating all changed items in the layout array at once. This produces exactly one React re-render per frame, regardless of how many items changed. Worst case during measurement window: 3 rounds × 1 full RGL re-render per round = 3 full layout computations for all items. Each round: observer fires for all changed items → one RAF → one setState → one RGL re-render. POC must log render count during measurement at 25 items and define pass/fail. After the measurement window closes, `computedMinH`/`computedMinW` updates are the ONLY state changes from observer callbacks — no `w`/`h` changes, so RGL repositioning is not triggered.
...
**Resize-overlap prevention:** Test: resize item A to be larger such that it would overlap item B. The expected ordering: RGL calls `constrainSize` first (which enforces content minimum), then applies its own collision detection to the constrained size. `constrainSize` only handles content minimums — it does NOT need to be aware of neighbors. RGL handles overlap prevention after. Verify this ordering in the POC by logging both `constrainSize` calls and `onResize` callbacks during a resize-into-neighbor scenario. If the ordering is different, document the actual behavior and adjust the architecture accordingly.
...
**Flag persistence:** The grid maintains an internal `Set<string>` for `userEdited` item IDs. An item is added to the set when the user drags or resizes it (detected via `onDragStop`/`onResizeStop`). The grid exposes TWO callbacks:
- `onLayoutChange(layout)` — fires on every layout change (including auto-sizing). Consumers should NOT persist this blindly.
- `onUserEdit(layout, editedIds)` — fires ONLY after user drag/resize. `layout` is the full layout array. `editedIds` is the Set of item IDs the user has touched. Consumers persist BOTH the layout and the editedIds set.

On re-mount, the consumer passes `initialLayout` (the persisted layout) and `editedIds` (the persisted set). Items in `editedIds` retain their persisted `x`/`y`/`w`/`h`. Items NOT in `editedIds` get auto-placed and auto-sized from content, ignoring the persisted values for those items. This cleanly separates user intent from auto-computed values — no derivation from layout shape needed.
...
**Edge case — multi-stage async loading:** The 200ms idle timeout is a heuristic. Widgets that load data in multiple stages (e.g., header at 100ms, rows at 300ms) may have the window close between stages. For these widgets, the layout config supports an optional `measureUntilReady: true` flag. When set, the grid keeps the measurement window open for that item until the widget calls a `reportReady()` callback (provided via React context). The 2s hard cap still applies. If a `measureUntilReady` item hits the 2s cap without calling `reportReady()`, a development-mode warning is emitted: "item 'X' has measureUntilReady but never called reportReady() — check that the widget calls reportReady() after data loads." This aligns with the "fail fast" philosophy.
